...
2. 明确本轮目标 `round_goal`，长期项目再对齐 `active_goal_summary`。
3. 建立或更新 Plan。
4. 判断证据、能力、协作拓扑和交付对象。
...
3. 把用户项目语境转成项目专用执行提示词，提示词内必须包含：项目目标、当前材料、交付契约、能力边界、子代理/线程策略、cadence、round_goal / active_goal_summary / 工具 Goal 生命周期关系、Loop 小循环、评分路由、证据和权限边界、恢复记录方式。
...
`round_execution_prompt` 至少包含：长期目标摘要、当前状态、本轮 `round_goal`、总规划约束、能力边界、协作拓扑、Loop 小循环、评分路由、交付契约和 next_route。每轮 Plan 必须说明：哪些来自总规划，哪些来自上一轮状态，哪些是本轮新增判断。若 Plan 只写当下任务、没有回扣总规划或 active_goal_summary，必须触发 route-back，先补 `round_execution_prompt`。
...
1. 读取最近一次机器看版，恢复 current_stage、current_basis、open_questions、decisions、next_route、round_index、master_prompt_location、active_goal_summary、topology_summary、capability_summary、codex_goal_lifecycle_status。
2. 先跑 `round_prompt_rehydration_gate`：从已确认总 prompt / active_goal_summary 和最新 state 压缩出本轮 `round_execution_prompt`，明确总规划继承、上一轮状态和本轮新增判断。
3. 再跑 `per_round_goal_lifecycle_gate`：把 Codex 工具 Goal 约束为本轮可完成的窄目标；长期连续性由 state、master prompt、closure-routing 和 next_route 承接，不用一个跨几十轮的工具 Goal 承接。
4. 再跑 `continuous_cadence_refresh_gate`：若 round_index 为 1、每满 3 轮、阶段/主链/交付对象变化、子线程输出失配、外部工具阻塞、用户新增关键词或版本号变化，则重新评估协作拓扑、能力清单、Goal 状态和 master prompt 是否需要局部 patch。
5. 只选择一个最高杠杆问题处理，优先处理会造成返工、幻觉、低摩擦失败或交付不可落地的问题。
6. 同步 Codex update_plan；Plan 必须基于 `round_execution_prompt`，说明来自总规划、上一轮状态和本轮新增判断的部分。用户明确触发连续节拍、直到停止、长期跟踪或 Goal 模式时，必须检查 Codex goal 生命周期；工具 Goal 默认服务当前 `round_goal`，完成即关闭，下一轮由 `next_route` 派生新的窄目标。不得把一轮 round_goal 完成误当成整个持续路线结束，也不得让 blocked 的旧工具 Goal 误判项目本身 blocked。
7. 写回协议、日志、代码或交付物中的必要位置。
8. 扫描关键节点，确认修改真实存在且章节、路由、证据、文件路径没有脱节。
9. 更新机器看版：stage_status、round_index、round_goal、round_execution_prompt_summary、codex_goal_lifecycle_status、goal_handoff_carrier、decision、evidence/status、topology_refresh_due、capability_refresh_due、goal_refresh_status、prompt_refresh_status、next_route、route_reason、manual_action_required。
10. 给用户一个短进度说明；如果没有阻塞且 continuous stop 条件未满足，下一轮从新的 next_route 继续。
